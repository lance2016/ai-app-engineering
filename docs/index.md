...
先用普通 Python 把机制讲清楚，末尾才对照 LangGraph、OpenAI Agents SDK、Claude Agent SDK 各自管这件事叫什么名字，以及哪一项三个框架都不管——通常是幂等、成本归因这类只能自己写的东西。

固定有三节：「常见错误」讲这个机制在生产里会以什么形态坏掉，讲取舍的那一节列的是没有标准答案、必须你自己做的判断（标题按各课的题目走，比如「松一点还是紧一点」「托管还是自部署」），框架对照表在末尾。其余的章节标题按主题走，不套模板：讲工具调用从一次重复扣款讲起，讲可观测性从「用户说它答错了，但没人知道错在哪」讲起。走一遍报文、日志或数字的例子折起来放，标题那行就写清是什么问题——已经信了的话点都不用点，直接往下读。一部分课还有作者在一个语音机器人项目上的经历，没有相关经历的课直接省略，不编。

单课 1～2.5 小时。示意代码二三十行，省略 import、日志和错误处理，不能直接运行——代码在这里是插图。

### 三种情况不适合

想 `git clone` 就能跑的项目，去[参考实现仓库](https://github.com/lance2016/ai-app-engineering-ref)，七个里程碑和这门课的 Part 一一对应；想学训练或微调模型，这里只讲到应用工程师做决策的深度；不写代码，正文全是机制和示意代码。
