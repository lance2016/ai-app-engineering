...
固定有三节：「常见错误」讲这个机制在生产里会以什么形态坏掉，讲取舍的那一节列的是没有标准答案、必须你自己做的判断（标题按各课的题目走，比如「松一点还是紧一点」「托管还是自部署」），框架对照表在末尾。其余的章节标题按主题走，不套模板：讲工具调用从一次重复扣款讲起，讲可观测性从「用户说它答错了，但没人知道错在哪」讲起。一部分课还有作者在一个语音机器人项目上的经历，没有相关经历的课直接省略，不编。
