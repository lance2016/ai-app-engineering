...
| 主线 | `lessons/` | 24 课 00–23，每课 `README.md + exercises.md + images/` |
...
## 心智模型
至少一张图（Mermaid 或 `images/` 配图）加几段话。这一节要能独立成立：
...
- 4～5 题，从动手改造到纯思考题递进。
- 每题配一个 `<details><summary>答案</summary>` 折叠块，答案要有讨论，不只是结果。
- **不引用不存在的文件**。要说「正文的 `xxx` 函数」，不说「`01_foo.py` 里的」。
- 至少一题是纯判断题（不写代码，读一段记录或做一个选择）。
...
- `complete`：九节齐全，示意代码有解释，练习有答案，链接可达
