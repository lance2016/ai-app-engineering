...
上面是生成线，下面是检索线。两条线的训练目标和使用位置不同：embedding 模型输出向量，指令模型输出文本或工具调用；两者不能只凭名字互换。
...
2. **「推理模型」可能是独立模型，也可能是指令模型的一种推理模式。** 产品命名并不统一，有的叫思考预算、reasoning effort 或 extended thinking。选型时要看模型卡和 API：是否能开、会产生多少隐藏 token、延迟和价格怎样变化。
3. **推理模式会增加计算成本。** 一些供应商会把隐藏的思考 token 计入用量，延迟也可能明显增加。意图分类这类短任务先用普通模式，真正需要多步推导的节点再开；是否值得要用主线[第 01 课](../../../lessons/how-llms-work/README.md)的成本模型测。
4. **开放权重不等于开源。** 权重能下载，不代表训练数据和训练代码公开，也不代表你能随便商用。有的许可证限制月活规模，有的禁止用模型输出去训练别的模型，有的要求显著标注。**这是硬约束，不是打分项**——选型时先过许可证这一关，再谈能力。
5. **模型卡先看五项。** 上下文长度（能塞多少，[F04](../04-context-window-and-sampling/README.md)）；层数与 kv 头数（自部署时算显存，[F06](../06-kv-cache-and-inference/README.md)）；许可证（能不能用）；知识截止（哪些事它一定不知道，得靠检索补）；是否原生支持工具调用、结构化输出与图片输入（决定你要不要自己兜底解析，或另建一条视觉流水线）。
...
8. **托管 API 常见三种形态。** 厂商原生接口、OpenAI 兼容网关、云厂商托管。协议兼容可以降低迁移成本，但不保证只改 base_url 和模型名；用了某家的原生特性（特定的缓存控制、服务端工具），仍要为它单独写 adapter。这条就是[原则 12 模型是可替换的适配器](../../../principles/12-models-are-swappable-adapters.md)的现实依据。

**多模态不是第六类模型，而是模型能力的一项标志。** 同一个指令模型可能只收文本，也可能同时收图片、音频或视频；选型时要把输入模态写进约束，接口层还要处理不同的 content block。DeepSeek 当前 API 用 `deepseek-flash` 调用 V4.1 Flash，并把图片按尺寸换算成 token 计费；具体格式和限制见[视觉输入指南](https://api-docs.deepseek.com/guides/vision/)（访问日期 2026-09-10）。
...
**按榜单选模型。** 榜单测的是别人的任务。要在自己真正依赖的能力上跑探针，做法见主线[第 01 课](../../../lessons/how-llms-work/README.md)。

**把「开放权重」当成「随便用」。** 许可证是法务问题，出问题时改模型的成本远高于选型时多读十分钟。

**拿模型卡上的上下文长度当可用长度。** 标称 128k 的意思是「塞到这么长不报错」，不是「在这么长上质量不掉」，更不是「你付得起」。长上下文里中间部分更容易被忽略（[F04](../04-context-window-and-sampling/README.md)），而且历史每一轮都要重发（[第 01 课](../../../lessons/how-llms-work/README.md)的成本模型）。**窗口是上限，不是目标。**
