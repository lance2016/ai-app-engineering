...
**拿模型卡上的上下文长度当可用长度。** 标称 128k 的意思是「塞到这么长不报错」，不是「在这么长上质量不掉」，更不是「你付得起」。长上下文里中间部分更容易被忽略（[F04](../04-context-window-and-sampling/README.md)），而且历史每一轮都要重发（[第 01 课](../../../lessons/01-how-llms-work/README.md)的成本模型）。**窗口是上限，不是目标。**
