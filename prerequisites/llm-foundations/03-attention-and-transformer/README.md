...
2. **一次完整的 self-attention 计算需要比较 n×n 对 token，复杂度是 O(n²)。** 上下文从 4k 到 32k，相关度计算量约是 64 倍。这是长上下文变贵的主要原因之一。
...
7. **GQA（Grouped-Query Attention）让多个 Query 头共享一组 K/V。** 它可以减少 KV cache 和相关带宽开销；质量变化取决于训练和具体配置，是否采用要看模型卡和实测。
8. **「lost in the middle」是实测现象：模型对上下文开头和结尾更敏感，中间部分可能更容易被忽略。** 重要信息的摆放要用自己的任务样本验证，检索结果也要留出重排和压缩空间。
9. **Flash Attention 等优化改的是计算路径，不改变 attention 的定义。** 它们通过分块和避免存完整的 n² 中间矩阵，减少显存读写；不同实现可能有微小的浮点误差。选托管 API 时看不到它，自部署时它会影响可用的上下文长度。
...
13. **decoder-only 在通用文本生成中占主流，因为训练目标直接对应续写。** encoder-only 和 encoder-decoder 仍适合分类、表示学习或输入输出分离的任务，不能只按流行程度选架构。
14. **MoE（Mixture of Experts）把 feed-forward 换成多个「专家」，每个 token 只走其中几个。** 「总参数 400B、激活 17B」这种描述就是 MoE。每 token 的计算量可能低于同总参数的 dense 模型，但路由、通信和装载全部权重的成本仍要算。
15. **模型卡片上的「上下文长度」通常是模型和服务端支持的上限。** 硬件、批处理和推理实现也会限制实际可用长度；超过上限可能被拒绝，达到上限也不代表质量稳定。
...
- O(n²) 和 lost in the middle → [第 08 课 Context Engineering](../../../lessons/context-engineering-for-agents/README.md) 的所有裁剪决定。
- 参数量与显存、MoE 的「激活参数」 → [第 23 课 模型适配与推理服务](../../../lessons/model-adaptation-finetuning-inference/README.md) 的选型。
