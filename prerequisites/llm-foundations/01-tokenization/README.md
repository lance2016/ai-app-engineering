...
2. **中文字符和英文词的切分方式差异很大，而且依赖词表和上下文。** 同样的意思，在不同 tokenizer 上可能得到不同数量的 token。做成本估算时不要把英文经验直接套到中文。
3. **字符数只能做很粗的预算草稿。** 代码、数字、空格、标点和混合语言都会改变结果；精确数字要用目标模型的 tokenizer 计算。
4. **数字、代码、JSON 常带来额外的 token 开销。** JSON 里的引号、括号、空格也会计入；结构化输出可能比自然语言描述同一信息更费 token，但换来了可校验的格式。这是主线第 02 课的取舍之一。
...
7. **位置信息要另外编码。** attention 本身不携带顺序，所以模型要知道「这是第几个 token」。绝对位置编码有长度上限；RoPE 这类相对位置编码常用于长上下文模型，但能否扩展还取决于训练和推理实现。
...
跑完看两件事：高频的字节串很早就被合并成一个 token；低频的到最后还是碎的。这就是「常见片段更短、罕见片段更碎」的机制。真实 tokenizer 的词表和合并规则因实现而异，原理不变。

## 常见错误

**用一个固定比例估 token。** 中英文、代码和数字的切分差异很大，固定比例只能做临时预算。要精确就用供应商的 tokenizer 或 token 计数接口，并在自己的常见文本上抽样校准。

## 它在 AI 应用里用在哪

- token 计费和估算 → [第 02 课](../../../lessons/model-api-structured-output-streaming/README.md) 的成本记录、[第 21 课](../../../lessons/reliability-cost-llmops/README.md) 的成本预算。
