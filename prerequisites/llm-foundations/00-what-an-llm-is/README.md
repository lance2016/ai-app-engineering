...
一个 LLM 就是一个函数：输入一串 token，输出「下一个 token 是什么」的概率分布。生成一段话就是把这个函数反复调用若干次。应用里很多边界，都能从这个循环推出来。
...
4. **上下文窗口是请求的硬边界。** 每次请求都受模型和服务端的输入、输出 token 上限约束；超过后可能被服务拒绝，也可能由客户端先裁剪。窗口容量本身不是固定费用，实际费用和延迟取决于本次发送的 token 数。[F03](../03-attention-and-transformer/README.md)解释了长度带来的计算成本。
...
10. **推理成本通常按输入、输出 token 分开计价，单价和缓存规则依供应商而异。** 多轮对话里，历史若再次作为输入发送，就会再次计费。这是主线第 08 课 Context Engineering 和第 21 课成本控制的经济学基础。
...
跑几个不同的 seed。你会看到训练语料里没出现过、但每一步都「合理」的句子。LLM 只是把「数前一个词」换成了「用 Transformer 看前面更多 token」，基本循环相同。这个实验说明，调节随机性解决不了事实性错误，应用仍要用事实注入和工具调用降低风险。
