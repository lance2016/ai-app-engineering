...
| 项 | 档 | 这门课哪里用到 |
|---|---|---|
| 复杂度直觉（O(n) 还是 O(n log n)） | 必备 | 第 04 课先跑精确检索，再用延迟、吞吐和 SLO 决定是否建索引 |
...
第 04 课不设「多少条向量以内不用索引」的固定数字。同样的数据量，Python 循环、数据库扩展和专用向量库的耗时可能完全不同；先用精确检索做基线，再用自己的维度、过滤条件和并发压测。
