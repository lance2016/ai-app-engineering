...
| M1 | [API 骨架](./m1-api-skeleton/README.md) | FastAPI + Pydantic + pytest；健康检查、鉴权、SSE 流式、结构化错误、system prompt 版本化 | 实现 | draft |
...
| M5 | [生产化](./m5-production/README.md) | Golden set 回归、OpenTelemetry + Phoenix、限流、Fallback、成本统计、故障演练 | 实现 | draft |
| M6 | [综合设计](./m6-platform-design/README.md) | 多租户知识库 + 任务 Agent 平台的 RFC：容量、威胁模型、模型与推理选型、迁移与退出 | 设计 | draft |
...
| [01](../lessons/01-how-llms-work/README.md) | 模型能力边界的判断 | 无（理解性课程） | P01, P02 |
...
| [04](../lessons/04-embeddings-and-vector-search/README.md) | Embedding 与相似度检索 | M4.1 | P09 |
...
| [21](../lessons/21-model-adaptation-finetuning-inference/README.md) | 模型与推理选型决策 | M6 | 01, 19 |
...
- `src/` 存放最终形态；各 `mN-*/` 目录存放该阶段的增量说明和阶段性代码。
