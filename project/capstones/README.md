...
> 章节练习证明你懂一个机制，Capstone 证明你能交付一个系统。先把一个题目做深，再考虑扩展其他方向。四个题目共用同一套工程验收语言；第 1 个是本仓库提供的 reference-grade 标准答案，其余三个保持为可扩展 outline。

## 四个题目

| # | Capstone | 一句话 | 前置 | 状态 |
|---|---|---|---|---|
| 1 | [Production Agent Service](./01-production-agent-service/README.md) | 把 M1～M5 交付成一个可上线的服务：一键部署、CI 门禁、评测报告、trace、故障演练、runbook | M5 | complete |
...
先完成 [Production Agent Service](./01-production-agent-service/README.md)，它覆盖最完整的交付面：功能、持久化、评测、观测、安全、成本和文档。只有当它的验收清单已经变成自己的测试和 runbook 后，再按兴趣选择 RAG、Durable 或 Multi-tenant 方向。
