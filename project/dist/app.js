(() => {
  ...
  <div class="workspace-head">
    ...
    <div class="head-summary">
      ...
      <div class="summary-value">${state.files.length} \u4E2A\u6587\u4EF6 <span>\xB7</span> ${allSheets().filter(({ sheet }) => sheet.included).length} \u4E2A Sheet</div>
...
`;
  var renderHeaderPicker = () => {
    ...
    <section class="panel header-panel">
      <div class="panel-title-row">
        <div>
          <div class="panel-kicker">\u8868\u5934\u8BBE\u7F6E \xB7 ${escapeHtml(file.name)} / ${escapeHtml(sheet.name)}</div>
          <h2>\u54EA\u4E00\u884C\u662F\u8868\u5934\uFF1F</h2>
        </div>
  ...
  <section class="results-section">
    <div class="results-heading">
      <div>
        <div class="panel-kicker">\u7ED3\u679C\u6982\u89C8 \xB7 ${(/* @__PURE__ */ new Date()).toLocaleDateString("zh-CN", { month: "2-digit", day: "2-digit" })}</div>
        <h2>\u91CD\u590D\u8BB0\u5F55</h2>
      </div>
...
`;
  ...
  var renderApp = () => {
    ...
    <footer class="app-footer"><span>${icon("check", 14)} \u53EA\u8BFB\u67E5\u770B \xB7 \u539F\u6587\u4EF6\u4E0D\u6539 \xB7 \u7ED3\u679C\u968F\u8BBE\u7F6E\u5373\u65F6\u66F4\u65B0</span><span>\u652F\u6301 XLSX / XLS / CSV / TSV</span></footer>
    ${state.toast ? `<div class="toast">${icon("check", 15)} ${escapeHtml(state.toast)}</div>` : ""}
  `;
  };
  var setToast = (message) => {
    state.toast = message;
    renderApp();
    window.setTimeout(() => {
      state.toast = null;
  ...
  var parseDelimited = (text, delimiter) => {
    const rows = [];
    let row = [];
    let cell = "";
    let quoted = false;
  ...
  var parseFile = async (file) => {
    ...
    const workbook = readSync(await file.arrayBuffer(), { type: "array", cellDates: true, raw: false });
    return { id: fileId, name: file.name, size: file.size, sheets: workbook.SheetNames.map((name, index) => ({ id: `${fileId}-sheet-${index}`, name, included: true, headerRow: 0, rows: utils.sheet_to_json(workbook.Sheets[name], { header: 1, defval: "", raw: false }) })) };
  };
  var loadFiles = async (files) => {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "select-sheet") {
      state.activeSheetId = target.dataset.sheetId;
      renderApp();
    }
    if (action === "toggle-sheet") {
      event.stopPropagation();
      const source = allSheets().find(({ sheet }) => sheet.id === target.dataset.sheetId);
      if (source) {
        source.sheet.included = !source.sheet.included;
        state.selectedResult = null;
        renderApp();
      }
    }
    if (action === "set-key-mode") {
      state.keyMode = target.dataset.mode;
      renderApp();
    }
    if (action === "set-header-row") {
      const source = allSheets().find(({ sheet }) => sheet.id === target.dataset.sheetId);
      if (source) {
        source.sheet.headerRow = Number(target.dataset.rowIndex);
        state.selectedResult = null;
        ensureColumnSelection();
        renderApp();
      }
    }
    if (action === "set-result-filter") {
    ...
    if (action === "toggle-mark") {
      ...
      if (state.marks.has(id)) state.marks.delete(id);
  ...
  document.addEventListener("change", (event) => {
    ...
    if (action === "toggle-column") {
      if (target.checked) state.selectedKeyColumns.push(target.dataset.column);
      else state.selectedKeyColumns = state.selectedKeyColumns.filter((column) => column !== target.dataset.column);
      renderApp();
    }
  ...
  document.addEventListener("dragover", (event) => {
    if (event.target.closest(".main-column, .sidebar")) event.preventDefault();
  });
