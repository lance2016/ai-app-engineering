...
> 中文写作，不绑定任何 Agent 框架，附带语音机器人方向的真实案例。

**状态：第一版内容已就位。** 24 课、12 条原则、12 个前置模块全部有正文和可运行代码；主项目 M0 有代码，M1～M6 是设计稿；三条方向选修中网络篇完整，另两条是大纲级草稿。状态列见各表，`complete` 表示代码能跑、练习有答案。每课标题旁的状态列告诉你哪些课已经能学。
...
| [generative-ai-for-beginners](https://github.com/microsoft/generative-ai-for-beginners) | GenAI 应用通识 | 跳过通识，直接从应用工程切入 |
| [llm-course](https://github.com/mlabonne/llm-course)、[LLMs-from-scratch](https://github.com/rasbt/LLMs-from-scratch) | 模型原理与训练 | 只取应用工程师需要的那一层，作为选修 |

## 路线图

```mermaid
flowchart LR
    PRE[前置<br/>Python 与后端基础] --> P0[Part 0<br/>起步]
    ...
    P2 -.-> T1[Track<br/>语音机器人]
    P1 -.-> T2[Track<br/>LLM 原理]
    P0 -.-> T3[Track<br/>网络与公网]
```

零基础读者从 [前置｜Python 与后端基础](./prerequisites/README.md) 开始，12 个模块学完再进主线。

Part 2 和 Part 3 没有硬依赖，可以并行或按兴趣先后。

三条轨道：**主线** `lessons/` 按编号学；**横向贯穿** 评测、安全、可观测、成本从第一个项目就带着，Part 4 再系统深化；**方向选修** `tracks/` 按岗位挑。阶段依赖和能力阶梯见 [ROADMAP.md](./ROADMAP.md)。

## 课程总表

前置模块见 [prerequisites/README.md](./prerequisites/README.md)，有后端经验的人做完那里的自检可以直接跳过。

状态：`outline` 只有目标和心智模型 · `draft` 有代码，缺练习或反例 · `complete` 可以学。
...
| **Part 1 模型与上下文** | | | |
| 01 | [LLM 工作原理与能力边界](./lessons/01-how-llms-work/README.md) | 用应用工程师需要的深度讲清 token、attention、上下文窗口、采样和幻觉，知道模型为什么能做又为什么会错 | complete |
...
| 04 | [Embedding 与向量检索基础](./lessons/04-embeddings-and-vector-search/README.md) | Embedding 是什么、相似度怎么算、pgvector 怎么建索引，为 RAG 和 Memory 打底 | complete |
| **Part 2 Tool 与 Agent** | | | |
...
[AI 应用服务骨架](./project/README.md)：从 asyncio 实验开始，七个里程碑，每个只加一个能力，最终长成带工具、RAG、Memory、评测和可观测性的生产级服务。每课的「对照真实项目」小节都指向这里。
...
| Track | 内容 |
|---|---|
| [语音机器人 Agent](./tracks/robotics-voice/README.md) | ASR/TTS 流式管线、打断、Realtime、双模型 racing、设备端工具 |
| [LLM 原理补课](./tracks/llm-internals/README.md) | Transformer、KV Cache、量化、推理服务，到「能做决策」为止 |
| [网络与公网访问](./tracks/networking/README.md) | TCP/TLS、DNS、反向代理、WSS、Tunnel、VPN |

## 怎么用这门课

1. 零基础从 [prerequisites/](./prerequisites/README.md) 开始；有后端经验的先做那里的自检，缺哪个模块补哪个。
...
4. 不要只读。每课的「对照真实项目」小节要求你在 `project/` 里落一个增量。
