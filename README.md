...
[背景知识](./prerequisites/README.md)都是**可选的**，不是必修：工程能力目录先解释读主线需要的最低概念，工程能力进阶目录集中处理分布式、可靠性和架构取舍；LLM 原理八篇在主线需要时会被点名引用。
不确定自己缺不缺什么，看一眼[背景知识](./prerequisites/README.md)；每个 Part 在搭什么、学完怎么算过关，见[课程总览](./lessons/README.md)。

## 27 课

每课 1～2.5 小时（第 00 课半小时），结构固定：为什么需要 → 怎么理解它 → 机制拆解 → 常见错误 → 取舍 → 工程落地 → 框架映射 → 一线经验 → 参考实现。
...
├── prerequisites/    背景知识：工程能力、工程能力进阶、算法基础、LLM 原理八篇
