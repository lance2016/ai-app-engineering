...
<summary>例子：用户替同事问了一句，三个月后 Agent 还坚持说「你不吃辣」</summary>

一次对话里，用户说的是：

> 帮我看看这家店，我同事不吃辣，得有几个不辣的菜。

提取那一步把它变成了一条记忆：

```json
{"content": "用户不吃辣", "kind": "preference"}
```

三个月后的另一次对话，模型接连推荐了三家清淡的店。用户问「你怎么知道我不吃辣」——答不上来，这条记录里除了这句话什么都没有。用户说「我从来没说过」，也没法核对。用户说「把这条删掉」，你不知道该删哪一条，因为「不吃辣」还可能从别的对话里被提取过第二次。

一条记错的偏好本身不算大事。麻烦的是它进库之后就成了「事实」：每次检索都可能被捞出来，捞出来一次就多一层看起来的可信度，而链路上没有任何一处能追回它是怎么来的。
