...
> 前面的课教你怎么把每一层做对。这一课处理动手之前的决定：自建还是买、检索还是微调、Workflow 还是 Agent、单体还是平台。决定要能被别人复算，也要写清什么情况下重开。
...
算出来：平均 QPS 0.14，峰值不到 1；原始向量常驻内存 40 万 × 1024 维 × 4 字节 ≈ 1.6G，近似索引本身还要再占一块，量级上按 2G 出头准备。从内存和 QPS 粗看，单机可以作为试点；高可用、备份、索引构建和故障恢复还要单独核算。托管方案一年的订阅费不够付半个人的工资。
...
技术选型不是列出最多方案，而是找出会改变结论的假设。没有容量、成本、威胁和退出条件的方案，很难被团队复盘，后续想替换也缺少依据。
...
2. **列方案时优先包括「不做」和「最简单的那个」。** 很多时候人工加规则就够，AI 方案要说明自己增加了什么价值。
3. **算数。** 日请求量、峰值 RPS、并发运行数、每日 token、每日花费、数据库写入。数不用精确，但要有，且每个数字能追溯到一个假设。
4. **决策矩阵的有用产出是敏感性。** 哪一个权重挪 10% 结论就翻？那就是你现在最该去验证的假设，而不是继续开会。
5. **ADR 记决定，更记后果和退出条件。**
...
下面几段代码只为说明容量和敏感性怎么算，省略了 `Estimate`、`Criterion`、`rank` 以及价格比例的定义，不能直接运行。
...
下面的后两行是在上一行基础上继续叠加一个杠杆，不是分别与基线比较。
...
!!! note "构造的容量例子"
    这组请求量、token、价格和并发数是为了展示计算关系编的，不是参考项目的生产指标。接入真实业务时，应该用账单、延迟和峰值流量替换它们。
...
!!! note "构造的 ADR 片段"
    文档量、团队人数、召回率阈值和 `+150ms` 都是为了展示写法的假设，不是参考项目的线上指标。真实项目应换成自己的评测和账单数据。
...
这是决策矩阵比较不容易被滥用的用法。先有答案再填分数，矩阵就只是装饰。
...
**平台化太早。** 第一个消费者还没稳定就抽象成平台，抽象层按想象中的第二个消费者设计，第二个真出现时发现不匹配。单体可以作为起点，等真实的第二个团队提出不同需求再抽象。

## 可逆性、速度和锁定成本

- **精确 vs 及时。** 早期估算的目的是判断量级和翻盘点，可以先保留范围和余量，再用上线数据校准；把小数位填得很满，不会自动带来更高的准确度。
...
- **「什么都不做」要真的评估。** 它可能避免新增工程成本，但也有不解决问题的机会成本和风险。把“不做”与最简单的人工或规则方案一起列出，写清各自承担的后果。
