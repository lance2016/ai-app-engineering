...
算出来：峰值 QPS 不到 1，索引常驻内存 1.6G，一台机器绰绰有余；托管方案按这个量报价，一年的钱不够付半个人的工资。

自建那一派赢了。但这十分钟最有用的产出是另一样东西——「日活 3000」这一行。它是产品目标，不是已经发生的事实。日活如果是 3 万，两边的数字全部要重算。

所以这份记录最后一行写的是退出条件：**日活过 1 万，或者 chunk 数过 300 万，这个决定重开。**
...
- **怎么测。** 估算部分能测：把假设表写成代码里的常量，断言 Little's law 那一步算出来的并发运行数和手算一致，改一个假设时哪些结论跟着变也就看得见了。ADR 部分测不了，但退出条件可以：每条退出条件对应一条告警，那条告警本身要有测试，否则条件被触发时没人知道。
