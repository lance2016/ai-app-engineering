...
放进视图：triage 的模型能"知道"发生了什么，回复更自然，但也可能把内部错误信息（"billing backend unavailable"）原样说给用户。不放进视图、由运行时决定怎么告知：可控，但 triage 的提示词要预留"收到兜底指令时怎么说"的分支。两种都行，关键是内部错误信息在到达用户前必须经过一层过滤，这是第 21 课的内容。
