...
一个发票处理流程，三步从一开始就是定死的：抽取字段 → 校验金额和税号 → 写进账务系统。团队把它实现成一个自治 Agent：三个工具都注册给模型，提示词里写「按顺序完成这三件事」。
...
|---|---|---|
| 步数 | 4 | 3 |
| 调用序列 | `extract` → `validate` → `validate` → `write` | `extract` → `write` |
| 结果 | 正确 | 一张税号有误的发票进了账务系统 |

第二次没有任何东西报错。模型看完 `extract` 的结果，觉得字段齐、格式对，就直接调了 `write`。提示词那句话它也不觉得自己违反了——它认为三件事做完了。
