...
主项目 [M5](../../project/m5-production/README.md) 的验收里有注入测试和越权测试：用 `01` 的方式构造几个带注入的文档放进 M4 的知识库，确认 M3 的 `ToolRunner` 守卫拦得住；用 `02` 的方式构造跨租户查询，确认 M4 的检索带租户过滤。[M6](../../project/m6-platform-design/README.md) 的威胁模型按本课的 OWASP 对照表逐条写。
