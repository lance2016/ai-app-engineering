...
**指令、数据、问题是三种东西。** 用户上传的文档、检索回来的段落、工具返回的内容，都是数据，不是指令。分隔能降低提示注入的成功率，但**不能保证安全**——真正的防线是第 05 课和第 20 课的确定性守卫。
...
另一条：把「不能承认自己是 AI」这类硬约束写在 prompt 里，线上仍然偶尔漏。最后的做法是 prompt 里保留约束，但输出后再过一道确定性检查。这就是第 20 课要讲的「守卫在代码不在提示词」。
