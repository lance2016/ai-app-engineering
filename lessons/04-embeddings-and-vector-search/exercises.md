...
在 `01_toy_embeddings_and_cosine.py` 的 `embed()` 里把归一化那两行删掉，`cosine()` 里也把除以模长去掉（变成纯点积）。再给候选列表加一条很长的、和密码无关但重复很多常见词的句子。
...
`DIM = 64` 意味着所有词被 hash 到 64 个桶，不同的词会撞到同一个桶。把 `DIM` 改成 8 和 4096，观察 `02_topk_search.py` 的排序变化。
...
`03_chunking_changes_recall.py` 的按句切块让每块只有一句，上下文太少。实现一个"两句一块、重叠一句"的滑动窗口切块，比较三种方式下目标句子的排名和得分。
