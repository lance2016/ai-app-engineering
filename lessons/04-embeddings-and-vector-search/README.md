...
> Embedding 把一段文本变成一个定长向量，让"意思接近"变成"夹角小"。有了它，检索就是算最近邻。这一课用一个没有语义的玩具向量把机制讲清楚，再用真实 embedding 模型看差别，为第 13 课 RAG 和第 14 课 Memory 打底。

## 学习目标

- 能解释 embedding 是什么、余弦相似度算的是什么，以及为什么要先归一化
...
- [01 LLM 工作原理与能力边界](../01-how-llms-work/README.md)：token 和窗口
...
**向量表达的是训练出来的相似性。** Embedding 模型是一个小型神经网络，训练目标是让意思相近的文本向量靠近。所以"忘记登录密码"和"重置密码"接近，而"重置密码"和"重置路由器"虽然共享两个词却离得远。本课的玩具向量只数词，正好把这个差别衬托出来。

**余弦只看方向。** 两个向量的夹角越小余弦越接近 1。先做 L2 归一化，余弦就退化成点积，算得快，也让长文本和短文本可以比较。pgvector 的 `<=>` 算的是余弦距离，等于 1 减余弦相似度。
