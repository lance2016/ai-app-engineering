...
- **怎么测。** 准备一组「查询 → 应该召回哪几条」的样本，量 Recall@k：前 k 条里捞回了几条该捞的。改切块、换 embedding 模型、调索引参数，都跑这一组。没有这个数字，一切检索优化都是感觉。第 13 课会在同一组样本上再加答案质量的评测。
- **`doc_id` 上要有索引**，因为按文档删除是高频操作（用户删文档、合规删除）。
- **这些结论下一次用到是在第 13 课**，中间隔了八课。到那时如果记不清切块和召回的关系，回来看一眼「心智模型」就够，不用重读整课。
