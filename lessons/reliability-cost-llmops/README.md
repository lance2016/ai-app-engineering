...
<summary>例子：供应商抖了 90 秒，服务不可用 11 分钟</summary>

某天下午，模型供应商有一段约 90 秒的 5xx。应用这边配了重试：最多 3 次，每次固定间隔 1 秒。

| 时刻 | 发生了什么 |
|---|---|
| `14:02:10` | 供应商开始返回 503 |
| `14:02:11` | 每个进行中的请求各自开始重试，打向供应商的 QPS 从 40 变成 160 |
| `14:02:30` | 供应商改返 429，响应头里写着 `Retry-After: 30`。重试逻辑没读这个头，照旧 1 秒一次 |
| `14:03:40` | 供应商恢复 |
| `14:03:41` | 积压的请求一起醒来——它们等的是同一个固定间隔——瞬时 QPS 打到 600，再次触发 429 |
| `14:13:20` | 人工把重试开关关掉，服务恢复 |

供应商坏了 90 秒，用户感受到的是 11 分钟。这段时间里应用这边的模型调用没有一次成功，但线程池被等待占满，连不需要模型的接口也开始超时。
...
两个容易混的概念：重试处理的是「这次失败了，再试可能成功」；熔断处理的是「连续失败了，再试只会浪费时间」。重试在单个请求内部，熔断跨请求共享状态。两个都要有，顺序是熔断器包着重试。
...
**容器。** 一个 Dockerfile 把依赖冻结成镜像。要点是分层缓存和不带密钥：
...
**CI。** 每次提交跑测试和评测门禁（第 19 课）。分数低于基线就让 PR 红掉。

**配置与密钥分离。** 模型名、价格表、预算上限、限流参数是配置，随环境变化，放配置文件或配置中心；API key、数据库密码是密钥，放密钥管理服务。两者都不进代码。

**灰度与回滚。** 换模型版本、改系统提示、调检索参数，都算发布。先给 5% 流量，看第 20 课的指标和第 19 课的评测分数，没问题再放大。
