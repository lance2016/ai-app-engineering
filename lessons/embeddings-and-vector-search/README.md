...
暴力法把查询向量和每一条比一遍，精确、简单、召回 100%。什么时候该换成近似索引，**没有一条「多少万条以内」的分界线**：同样十万条向量，Python 循环、NumPy 批量、pgvector、专用向量库的延迟能差几个数量级，还要看维度、内存、并发和过滤条件。顺序是：跑精确检索 → 压出延迟和吞吐 → 和 SLO（你对用户承诺的延迟上限，第 21 课细说）比 → 超了再上索引。
