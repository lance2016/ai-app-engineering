...
<summary>例子：客服机器人聊到第 48 轮收到一条 400，聊得短的用户从来没事</summary>
...
选型没做完的账，三个月后会一起来：某类输入模型一直做错，没人知道边界在哪；账单比预想高一个量级，因为没人算过历史重发；想换模型时才发现提示词、解析逻辑和厂商特性全绑在一起。这一课把选型做成三段可复现的代码——硬约束过滤、成本模型、能力探针，加起来不到一百行。
