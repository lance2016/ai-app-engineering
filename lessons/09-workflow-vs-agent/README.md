...
主项目 [M3](../../project/m3-tool-workflow/README.md) 是一条"校验、确认、执行"的确定性链加一个模型决定下一步的循环，每一步的守卫和停止条件都在代码里。它刻意不做自治规划，因为每个副作用都要过确认门，路径可枚举。M3 README 的「选型记录」一节写了这个理由，也写了为什么暂不拆 router + worker。
