...
而这次调用在监控里是成功的：HTTP 200，`finish_reason` 是 `stop`，用量也照记。协议层没坏，模型也没拒答，坏的是值——**能解析出来和值是对的，是两件事**，这一课有一半篇幅在讲这条缝怎么补。

!!! note "构造的例子"
    这段 JSON 和后面几段日志、事件流都是为讲清机制构造的，不是某次线上返回的记录。「TTS 要边收边说，设备命令必须等全」那一节是真事。

另一半篇幅给的是同一次调用的其他坏法：流可能中途断、重试可能重复计费、用量可能压根没打开。把消息、schema、增量和用量拆开看，才知道故障出在协议、解析还是供应商。
...
每条消息有角色。系统消息放指令，用户和助手消息交替，工具结果是单独一种角色，靠一个调用 id 和助手那条里的调用对上，而不是靠顺序。`content` 建议设计成一串带类型的块——图片和推理模型的思考都以块的形式待在里面，见下面第二节。
...
① 供应商原生的结构化输出，服务端按 schema 约束解码；② 自己把 JSON Schema 写进提示，返回后在客户端校验；③ 校验没过，再决定要不要回喂给模型改。**第二层永远不能省**，哪怕第一层已经开了：约束解码管的是语法，管不了值对不对——开头那段 JSON 就是第一层拦不住的东西。
...
**但开了它，下面这套「一份 schema 用两次」照样要留着。**约束解码保证的是「解析得出来」，保证不了「值是对的」，开头那段 JSON 就是从这条缝里漏下去的：
...
同一份 `Invoice` 用了两次：一次生成写进提示里的 schema，一次校验返回值。开头那三个错字段——数字类型、日期格式、金额逗号——`date` 的 `pattern` 和 `total` 的 `float` 一次全接住，Pydantic 报的第一条错误原文直接发回去，第二次基本就对了。
...
开头那个 `total` 正好落在第一行：`"1,280.50"` 去掉逗号还能自动修，但如果模型返回的是 `1280.05`，格式合法、校验通过、值错一位，只有对账能发现。这些字段校验没过就该拒绝，把错误抛给上层或转人工。**能自动修的是形状，不能自动修的是事实和授权。**这条边界是原则 01 在解析层的形态。
...
- **怎么测。** 三层方案各测一层。校验层拿一组故意坏的 JSON（缺字段、类型错、枚举越界、外面包了代码围栏，开头那段就是一条），断言每一类都走到「修复重试」而不是抛异常。流式层拿一段录好的 SSE 事件流回放，断言文本增量边到边发出去、结构化参数攒完整校验过才交出去、`usage` 不在最后一个事件上时不崩。录好的事件流不需要供应商，跑得比真调用快两个数量级（第 18 课）。
