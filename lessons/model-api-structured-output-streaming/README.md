...
<summary>例子：HTTP 200、finish_reason=stop，返回的三个字段全错</summary>
...
而这次调用在监控里是成功的：HTTP 200，`finish_reason` 是 `stop`，用量也照记。
...
协议层没坏，模型也没拒答，坏的是值——**能解析出来和值是对的，是两件事**，这一课有一半篇幅在讲这条缝怎么补。
...
<details class="case" markdown="1">
<summary>例子：开头那个 total 落在第一行，所以修不了</summary>
