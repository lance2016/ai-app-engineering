...
还有两件 SDK 不替你做的事。重试要按错误类型分，不是按次数分：「再试一次结果可能不一样吗」是唯一的判断标准。用量要每次调用都记下来，因为账单月底才有。这一课只管把 usage 从 API 里拿出来并落库；这些 token 怎么变成钱，第 01 课算过；限流、熔断、降级到备用模型，第 20 课。
...
上面把 `content` 写成字符串是简化。这门课的内部表示里它是一串块，每块有自己的类型：
