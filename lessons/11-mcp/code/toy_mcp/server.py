"""Minimal MCP-shaped server: JSON-RPC 2.0 over stdio, one JSON object per line.

Implements the subset a client needs to discover and use capabilities:
initialize, notifications/initialized, ping, tools/list, tools/call,
resources/list, resources/read. Message shapes follow the MCP spec
(protocol revision 2026-07-28); everything else is left out on purpose.

Usage (normally launched by the client):
    python server.py [--read-only] [--crash-on METHOD]
"""
...
NOTES = {"notes://todo": "buy milk\ncall mum", "notes://ideas": "write an MCP lesson"}
...
def handle(msg: dict, *, read_only: bool, initialized: bool) -> tuple[dict | None, bool]:
    ...
    if method == "initialize":
        ...
        return result(req_id, {"protocolVersion": PROTOCOL_VERSION, "capabilities": caps,
                               "serverInfo": {"name": "toy-notes", "version": "0.1.0"}}), initialized
    if method == "notifications/initialized":
        return None, True  # notification: no response
    if not initialized:
        return error(req_id, -32600, "server not initialized"), initialized
    if method == "ping":
        return result(req_id, {}), initialized
    if method == "tools/list":
        visible = {n: t for n, t in TOOLS.items() if not (read_only and t["side_effects"])}
        return result(req_id, {"tools": [{"name": n, "description": t["description"], "inputSchema": t["inputSchema"]} for n, t in visible.items()]}), initialized
    if method == "tools/call":
        name = params.get("name")
        if name not in TOOLS or (read_only and TOOLS[name]["side_effects"]):
