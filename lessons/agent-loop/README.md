...
> 许多 Agent 的核心是一个循环：模型提出下一步，运行时执行，把结果放回去，再问模型。这一课讲这个循环里哪些事归模型、哪些事归运行时，尤其是「什么时候停」——模型只能提出停止意图，停下由运行时执行。

<details class="case" markdown="1">
<summary>例子：用户说了三遍「不聊了」，机器人每次答应完又起了个新话题</summary>
...
修法是加一个显式的退出工具。模型判断用户想结束时调用它，运行时收到这个调用就静默终止本轮会话。模型仍然负责判断，但停不停由代码决定。这比把提示词写得更严稳定得多。
...
- [05 Tool Calling](../tool-calling/README.md)：工具契约和六个守卫，本课的循环建立在它们之上
...
模型只回答一件事：下一步做什么。执行工具、把结果记回消息、判断该不该继续、失败了怎么处理，这四件都是代码在做。这条边界画清楚之后，Agent 的可靠性就落到普通软件的那些问题上。

这个「模型想一步、运行时做一步、结果再回给模型」的形状有个名字：**ReAct**。它出自 2022 年的[同名论文](https://arxiv.org/abs/2210.03629)，很多 Agent 框架都提供它的变体。知道这个名字，读别家文档时能认出自己在看什么。

### 停止条件全部由运行时持有

模型「自然停下」是一种停止方式，但不能是唯一的。至少还要有步数上限、token 预算、时间预算，以及跑偏检测。

聊天场景里连「自然停下」都不成立：模型的倾向是接着聊，所以那条路径根本不会走到。退出工具补上的是一条**模型能主动走、但由运行时执行**的停止路径，它和上面那几条预算是并列关系，不是替代关系。

每种停止都要有名字，让用户和日志知道为什么停的。

### 失败要分类，恢复要确定

网络抖动重试就行；参数错了回给模型让它改；模型反复调同一个工具，再问它一次只会得到同样的结果，应该警告一次然后转人工。三种情况用同一招「再问模型」，结果是烧钱、绕圈、然后超时。

### 一个 Agent 经验上管 3～10 步

上下文变长时，模型跑偏的风险通常会上升，这是 12-factor 的 factor 10 反复强调的经验。任务大就拆成多个小 Agent，让确定性代码把它们串起来。这一点第 09 课和第 11 课展开。

拆不开的长任务还有另一条路：循环不用改，给它一份每轮重新念一遍的清单，再逐项验收。第 10 课讲这条路，以及两条路各自适合什么。
...
把「用户想结束」做成一个退出工具装进来，就是多一个分支：`run_tool` 认出这个工具名，不执行任何副作用，直接让 `run_agent` 带着 `stop_reason=USER_ENDED` 返回。它和 `STEP_LIMIT` 一样是运行时的一条停止路径，只是触发它的信号来自模型。
...
if sig in seen:
    if warned:
        return "needs_human"          # 警告过一次还重复，转人工
    warned = True
...
**把「什么时候停」写在提示词里，没有对应的代码分支。** 提示词里的停止条件是一句建议，运行时那边得真的有一条路径在接它；没有代码接，模型判断得再对也停不下来。
...
- **步数上限设多少。** 太小任务做不完，太大失控成本高。经验值按任务类型分档：查询类 3～5，多工具协作 8～12，超过 15 步的任务应该拆。上限是安全网，不是目标。语音机器人那个项目里还有一层：任务型 Agent 的上限要比聊天型小得多，因为每一步都伴随一次设备动作，跑偏了设备就真的动了。
- **重试的代价谁付。** 重试是拿延迟换成功率。给用户的实时对话里，一次重试可能就超出可接受的等待时间；后台任务则可以多重试几次。所以重试次数要做成循环的参数，不要写成常量。
- **转人工，还是直接放弃。** 转人工需要有人接，有人接就要有第 07 课的暂停机制。没有这个机制时，诚实的「我做不到」比假装完成好，也比无限等待好。
...
三个框架都不替你做完整的失败分类和预算记账，这部分仍需要自己的代码。官方文档：[LangGraph](https://langchain-ai.github.io/langgraph/) · [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) · [Claude Agent SDK](https://docs.claude.com/en/api/agent-sdk/overview)（核对日期 2026-09-05）。
