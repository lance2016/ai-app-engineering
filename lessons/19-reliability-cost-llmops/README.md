...
注意这意味着**最多会超出一步的钱**。这在设计上可以接受，前提是你知道单步最大花费并留了余量。想严格不超，就要预估这一步的成本再决定要不要执行，代价是预估不准时会过早停止。
...
**CI。** 每次提交跑测试和评测门禁（第 17 课）。分数低于基线就让 PR 红掉。

**配置与密钥分离。** 模型名、价格表、预算上限、限流参数是**配置**，随环境变化，放配置文件或配置中心；API key、数据库密码是**密钥**，放密钥管理服务。两者都不进代码。

**灰度与回滚。** 换模型版本、改系统提示、调检索参数，**都算发布**。先给 5% 流量，看第 18 课的指标和第 17 课的评测分数，没问题再放大。
