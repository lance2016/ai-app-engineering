...
# 主线 27 课：每个 Part 在搭什么

> 27 课不是 27 个独立主题。它们按「一个 AI 应用是怎么一层层长出来的」排序：先有能调通的模型，再有能执行的工具和能循环的运行时，再有外部知识和记忆，最后是让它能上线、能被观测、能被信任的那一圈骨架。
...
[User<span>P5 · 24</span>](./product-design-ux/README.md){ .sysnode }<span class="sys__arrow" aria-hidden="true"></span>[API<span>P4 · 18</span>](./system-architecture/README.md){ .sysnode }<span class="sys__arrow" aria-hidden="true"></span>[Session<span>P2 · 07</span>](./agent-state-and-runtime/README.md){ .sysnode }
</div>
</div>
...
| 检索 | 不在模型权重里的知识怎么进来 | 04 · 15 · 17 | 只能回答通识问题，一问业务就编 |
| 工具执行 | 模型怎么影响外部世界，谁保证只发生一次 | 05 · 12 · 13 · 14 | 重试变成两笔转账；模型能调它不该调的东西 |
| 运行时控制 | 循环怎么走、什么时候停、状态归谁 | 06 · 07 · 09 · 10 · 11 | 死循环烧钱；重启之后不知道自己在做什么 |
| 上下文组装 | 每一轮到底发了什么给模型 | 08 · 10 · 16 | 模型「忘事」，而你无法解释为什么 |
| 评测 | 凭什么说这次改动变好了 | 19 | 每次上线都是赌博 |
| 可观测 | 出事之后能不能查出是哪一层 | 20 | 只能看到最后那句错误回答 |
| 可靠性与成本 | 下游抖动怎么办，钱怎么算清 | 21 | 上游一慢全站雪崩；月底看账单才知道 |
| 安全边界 | 谁能让它做什么，数据会不会漏 | 22 | 一段用户输入就能越权 |
| 架构与产品决策 | 这些东西怎么拼起来，用户看到什么 | 18 · 23 · 24 · 25 · 26 | 组件都对，系统不成立 |
...
每题先自己答一句话，再展开对照。**都答得上就不用通读这个 Part**，挑各课的「常见错误」和讲取舍的那一节看就够。答不上的，括号里是对应的课。
...
榜单测的是别人的任务。真实模型在自己的探针上往往参差：算术过了，数字母挂了；能写 JSON，但拒绝承认不知道。第一步是一组「一个提示加一个确定性检查」的探针，跑在自己真正依赖的能力上——但它只是排雷，说明不了模型在你业务上有多好，那要一份按真实请求采样的评测集（第 19 课）。
...
→ [01](./how-llms-work/README.md) · [21](./reliability-cost-llmops/README.md)
</details>
...
**学完之后。** 应用有了工具、循环、状态、上下文组装和能力接入，可以自己走多步完成一个任务；任务长到几十轮时，知道用清单和验收把目标兜住，也知道什么时候该换成拆分。
...
| [10](./long-horizon-tasks/README.md) | 目标漂移；清单怎么接进循环、怎么验收、什么时候改计划 |
| [11](./multi-agent-handoff/README.md) | 分工、交接和并行竞速；状态归谁、历史给多少 |
| [12](./mcp/README.md) | 能力接入：生命周期、能力发现、两条错误通道 |
| [13](./skills-and-capability-layers/README.md) | Tool、MCP、Skill、Plugin、A2A 各管什么 |
| [14](./agent-harness/README.md) | 编码 Agent 拆解：工具界面、权限分级、hook、上下文寿命 |
...
- 上下文窗口为什么是预算而不是容量（08）
- Agent 和 Workflow 的边界在哪，什么时候多一个 Agent 反而更差（09、11）
- 长任务里目标为什么会漂，清单靠什么机制兜住它，「完成」该由谁判定（10）
- MCP 解决了什么，不解决什么（12、13）
- 一条纪律该写进权限声明、hook 还是提示词，判断依据是什么（14）
- 编辑类工具为什么用「替换一段唯一文本」而不是整文件重写（14）
...
→ [15](./rag-end-to-end/README.md) · [19](./evaluation/README.md)
</details>
...
| 课 | 一句话 |
|---|---|
| [15](./rag-end-to-end/README.md) | 解析、切块、索引、混合检索、重排、生成、引用 |
| [16](./memory/README.md) | 会话、任务、长期记忆的边界；提取、冲突合并、过期 |
| [17](./data-engineering/README.md) | 版本、新鲜度、权限和删除演练 |

**出师标准。**

- 回答错了，怎么判断问题出在检索还是生成（15）
- Recall@k 和 Hit@k 分别回答什么问题，什么场景该看哪个（15）
- 引用校验能证明什么、不能证明什么（15）
- 记忆系统最危险的故障是哪一种，怎么测出来（16）
- 为什么说决定 RAG 上限的是数据不是模型（17）
...
→ [19](./evaluation/README.md) · [原则 08](../principles/08-no-eval-no-improvement.md)
</details>
...
→ [22](./security-governance/README.md) · [原则 11](../principles/11-guardrails-in-code-not-prompts.md)
</details>
...
| 课 | 一句话 |
|---|---|
| [18](./system-architecture/README.md) | 从客户端到模型再回来的完整请求链，以及存储边界 |
| [19](./evaluation/README.md) | 切片、kappa、轨迹断言、回归门禁 |
| [20](./observability/README.md) | 结构化日志、OpenTelemetry GenAI 约定、四种故障的样子 |
| [21](./reliability-cost-llmops/README.md) | 超时、重试、限流、熔断、fallback、成本预算、SLO |
| [22](./security-governance/README.md) | 提示注入、越权、数据泄露、沙箱、多租户边界 |
| [23](./model-adaptation-finetuning-inference/README.md) | 什么时候该微调、显存怎么算、自建的成本临界点 |
...
**学完之后。** 能把一个 AI 应用的关键交互、语音链路和技术决定写成可复核的设计文档。
...
→ [24](./product-design-ux/README.md)
</details>
...
**实际播出去的那一部分，不是模型生成的全文。** 用户只听到了前半句，后面的对话必须建立在前半句上。把全文写进历史，下一轮上下文就会包含用户从没听过的内容。

这类问题特别难查：文本 trace 上每一轮都自洽，模型输出也正常，只有用户觉得「它在胡说」。要发现它，历史里必须记下这一轮实际播出到哪里。

→ [25](./voice-agents/README.md)
</details>
...
→ [26](./system-design-decisions/README.md)
</details>
...
| 课 | 一句话 |
|---|---|
| [24](./product-design-ux/README.md) | 流式 UI 状态机、确认与撤销、引用展示、反馈闭环 |
| [25](./voice-agents/README.md) | 级联还是端到端、一秒的预算怎么分、打断之后历史记什么 |
| [26](./system-design-decisions/README.md) | 容量估算、决策矩阵的敏感性、带退出条件的 ADR |

**出师标准。**

- 一个流式界面要表达哪几种状态，哪一种最容易被漏掉（24）
- 什么操作必须要确认，什么操作可以只提供撤销（24）
- 语音链路那一秒预算分给了哪几段，哪一段最先该优化（25）
- 用户打断之后，对话历史里该记的是模型生成的全文还是别的（25）
