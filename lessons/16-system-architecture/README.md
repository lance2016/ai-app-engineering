...
这一课是主项目 [M1](../../project/m1-api-skeleton/README.md) 和 [M2](../../project/m2-state-and-storage/README.md) 的架构总结：M1 做了网关、鉴权和 SSE 端点，对应 [`aiapp/api/`](../../project/src/aiapp/api/) 的 `deps.get_tenant`、`errors.py` 和 `routes/threads.py`；M2 做线程持久化和 Redis 状态，对应 [`aiapp/storage/`](../../project/src/aiapp/storage/)：事实在 PostgreSQL 的 `message` 表，Redis 只放幂等键和运行锁，清空 Redis 对话不丢。学到这一课时回头看这两个里程碑，检查每一跳是不是都有对应的模块。[M5](../../project/m5-production/README.md) 会在这条链的每一跳加上 trace。
