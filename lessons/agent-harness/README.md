...
> 05 到 13 课的核心零件已经齐了：工具、循环、状态、上下文、能力接入。这一课把它们放进编码 Agent 这个具体场景里看一遍。它会直接改文件、跑命令、处理长会话，前面分开讲的边界会在这里同时出现。

<details class="case" markdown="1">
<summary>例子：「改完跑一遍格式化」写进提示词，一天 40 次编辑漏了 9 次</summary>

一个编码 Agent 上线两周后，团队定了条纪律：每改完一个文件跑一遍 `ruff format`。落地方式是在系统提示词末尾补一句：

```text
After every file edit, run `ruff format` on the file you just changed.
```
...
|---|---|
| `edit_file` 调用 | 40 |
| 后面跟了一次 `run ruff format` | 31 |
| 没跟 | 9 |

漏掉的那 9 次没有规律。有的是连着改了三个文件、最后只格式化了一个；有的是中间跑了测试、测试挂了，处理完就把这事忘了；有的是会话长到触发压缩，那句话没进摘要。

补救的第一反应是把提示词写得更重：加粗、挪到最前面、写成 MUST。命中率从 78% 涨到 90% 出头，然后不动了。

!!! note "构造的例子"
    这组次数是为讲清「提示词不是拦截点」编的，不是某天的真实统计。
...
模型要求删掉一个目录，该直接执行、问一句、还是拒绝？一次会话跑了两百轮，上下文早就满了，压缩之后它忘了自己在改哪个文件。团队定下的纪律，模型多数时候会遵守——多数时候不等于每次，而没人说得清漏的是哪几次。
...
**工具是给模型的界面，不是给人的 API。** 人改文件用 `sed`，模型不该。给模型的工具要满足三条：动作正交（读、改、跑、找，四个动词覆盖很多常见任务）、错误能自解释（告诉它哪里错了、下一步怎么办）、结果可预期。[SWE-agent](https://arxiv.org/abs/2405.15793) 把这一层叫 agent-computer interface，论文的实验说明：同一个模型换一套工具界面，任务成功率会随之变化。
...
**上下文有寿命，运行时要管。** 长会话迟早会撞窗口，三种办法：压缩（有损，最容易丢的是「还没做完的事」）、把内容挪到文件系统按需再读（第 08 课的 just-in-time）、派一个子 Agent 去干脏活，它的上下文和主循环隔离，只把结论带回来。三种办法都会改变主循环能直接看到的内容：压缩会丢细节，文件和子 Agent 把细节移到窗口外。区别在于信息保留在哪里，以及主循环能不能按需取回。
...
描述里那两句「不要用它做 X，改用 Y」是**工具之间的分工声明**。少了它们，模型会用 `run` 去跑 `cat` 和 `sed`，把另外三个工具的边界全绕过去——权限分级、审计、错误回喂跟着一起失效。

第三个参数是可逆性，它是下面权限门唯一的判断依据。工具在注册时就声明自己有多危险，而不是等运行时去猜。
...
零次和多次都不是崩溃，是给模型的下一步指令（第 05 课的错误回喂）。模型看到「出现了 3 次，把上下文扩几行」，下一轮基本就对了。
...
`mode == "plan"` 这个档位只管权限：允许读、禁止写，让模型先摸清情况再动手。它和第 09 课那个带 schema 的 `Plan` 没有关系——一个说的是能不能改东西，一个说的是要做哪几步。

拒绝和否决都变成工具结果回给模型，不是异常。模型知道自己被拦了，可以换一条路；抛异常只会让循环断在半路，用户看到的是一个栈回溯（第 06 课的失败分类）。
