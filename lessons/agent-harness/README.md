...
**工具是给模型的界面，不是给人的 API。** 人改文件用 `sed`，模型不该。给模型的工具要满足三条：动作正交（读、改、跑、找，四个动词覆盖绝大多数任务）、错误能自解释（告诉它哪里错了、下一步怎么办）、结果可预期。[SWE-agent](https://arxiv.org/abs/2405.15793) 把这一层叫 agent-computer interface，它的实验结论很直接：同一个模型，换一套工具界面，成功率差出一大截。
