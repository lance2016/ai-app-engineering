...
<summary>例子：机器人执行时不出声，用户以为没听见，又说了一遍，动作做了两次</summary>
...
等待、流式输出、工具执行中、需要确认、完成、失败，每个状态的渲染和可用操作都不同。开头那个机器人缺的就是这张图里的 `tooling`——用户听不出它在干活，于是又说了一遍。
...
每个状态要回答同一个问题：用户此刻看到什么、能点什么。答不上来的那个状态，就是下一次「用户以为没反应，于是又来一遍」的位置。
