...
> 前面二十三课都在让 Agent 可靠。这一课问的是另一件事：用户凭什么相信它、什么时候该把决定权还给用户、怎么知道产品有没有变好。答案大多不在模型里，在交互设计和反馈闭环里。
...
等待、流式输出、工具执行中、需要确认、完成、失败，每个状态的渲染和可用操作都不同。
...
- **怎么测。** 状态机写成一张转移表，测试就在表上跑，不用碰界面：断言每个状态都有出边（没有死状态）、断言 `TOOL_RUNNING` 这个状态存在（第一版漏的就是它，代价见下面一线经验）、断言任何通向不可逆动作的路径上一定经过确认状态。三条都是确定性的，能进 CI（第 18 课）。
