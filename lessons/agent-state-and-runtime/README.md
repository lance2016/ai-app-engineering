...
- **模型列的清单归哪一类。** 模型输出的「接下来要做这五件事」和工具结果一样，是发生过的事实，追加进线程就行。「其中哪几件做完了」则要从后续事件推导，别单独存一份跟着它同步——上面那条「执行状态另存一份」说的就是这个。清单要活过压缩是另一件事，归第 08 课的 `protected` 管。
...
- **怎么测。** 用一个记调用次数的假工具，跑一段跑到一半的事件线程，断言三件事：从 checkpoint 加载后已执行的工具没有跑第二遍、`to_messages()` 的输出和崩溃前逐字相同、两个写者并发 `append(expected_seq=n)` 只有一个成功。三条都不需要模型，一秒内跑完，能进 CI（第 18 课）。
