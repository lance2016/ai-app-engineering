...
运行时不维护「当前第几步、是否在等用户」这类变量，需要时从事件列表里算。这样存盘、恢复、界面展示和日志排障都以同一份事件列表为依据，少维护一套容易漂移的状态。
...
模型已经说了要做什么，运行时还没做。在这个点上存盘，恢复时先看待处理的调用：有结果的跳过，没结果的执行。这样多数进程崩溃点都能从事件继续。
...
`thread.append()` 返回它刚追加的事件，所以 `yield thread.append(...)` 一行同时做了两件事。这样流和内存里的事件对象一致；持久化仍要处理 `save()` 失败的情况，不能把一次成功的推送当成已经落盘。
