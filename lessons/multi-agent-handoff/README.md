...
一个客服系统，triage Agent 判断问题类型，然后转给专家 Agent。交接策略是「只给最后一句用户消息」——省 token，专家也不会被闲聊带偏。

| 轮次 | 谁 | 说了什么 |
|---|---|---|
| 1 | 用户 | 「我上周下的单 A-8871 被扣了两次款」 |
| 2 | triage | 「稍等，我查一下」 |
| 3 | 用户 | 「另外这单还能改地址吗？」 |
| 4 | triage | 「地址可以改，我先处理重复扣款」 |
| 5 | 用户 | 「好」 |
| 6 | triage | `transfer_to_billing(reason="duplicate charge")` |
| 7 | billing | 「请提供一下订单号」 |

策略是生效的：billing 拿到的就是第 5 轮那个「好」。订单号在第 1 轮，被裁掉了。
