...
**同一次调用的重试，和同一个业务意图被发起两次，是两件事，要用两个不同的键挡。** 只挡住前者，模型隔一轮重发同一个意图，副作用就发生第二遍。这是本课第四个守卫要解决的问题，也是三个主流框架都不替你做的那一项。
...
- **怎么测。** 断言不看模型说了什么，看它调了什么：工具名对不对、参数过不过 schema、有副作用的调用有没有走确认门、模型在下一轮重发同一意图时业务键有没有撞上。最后一条就是开头那两笔退款的回归测试。四条写成测试，就是第 19 课轨迹评测的最小形态。
