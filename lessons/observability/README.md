...
日志答不上来，不是因为记得太少。这八行记的是「我在做什么」，而每个问题要的是「这一步的输入输出和结果是什么」。八行之间也没有任何父子关系，看不出哪一次检索喂给了哪一次调用。

这一课要造的东西，就是让这张工单能在五分钟内定位到八层里的哪一层。
...
上面那八行日志，如果每一步都开一个 span、并且带上输入输出，它长这样：
...
回到工单 #4471：这棵树上第二个 `chat` span 的 `gen_ai.input.messages` 里能直接看到检索结果有没有进上下文，`execute_tool search` 的返回体里能看到召回了哪 5 条。两个问题各归一层，不用猜。
...
和开头那八行日志比，差别在两处。`event` 是一个稳定的短标识（`model.call`、`tool.call`、`run.finish`），不是一句话——稳定标识才能聚合，「过去一小时 `model.call` 的 p99 延迟」是个能查的问题，「过去一小时打印了『正在调用模型...』的次数」不是。另一处是 `run_id` 每一行都带，有了它，日志流才变成一次运行的故事。
