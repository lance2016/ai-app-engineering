...
- **怎么测。** 显存估算写成一个函数，拿几个已知配置对一遍（参数量、精度、批大小、上下文长度 → 显存），偏差超过一成就是公式抄错了，这条能进 CI。微调和量化的效果只能靠第 18 课的评测集，而且必须按切片比：int4 的质量下降不均匀，平均分掩盖了某些切片的崩塌。
...
[← 上一课 21](../security-governance/README.md) · [下一课 23 →](../product-design-ux/README.md)
