...
<summary>例子：为「答得不准」微调了三周，上线后反馈没变，根因在检索</summary>

一个内部知识问答，用户反馈「答得不准」。团队的判断是模型不懂业务术语，决定微调。

三周：标了 1400 条样本，跑了一轮 LoRA，部署了自己的推理服务。上线之后，用户反馈没有变化。

回头做的第一件事，是把 30 条被投诉的问题逐条摊开，看每一步的输入输出：

| 问题出在 | 条数 |
|---|---|
| 检索没召回到正确的那一段 | 19 |
| 召回到了，但被切块切碎，只剩半条规则 | 7 |
| 上下文里有正确答案，模型答错 | 4 |

只有最后 4 条是模型的问题，而这 4 条里有 3 条换个提示词就对了。

三周花在了占 13% 的那一段上。更贵的是留下来的东西：一个要跟着基座升级重训的模型，加一套要自己运维的推理服务。
...
**[LoRA](https://arxiv.org/abs/2106.09685) 在这棵树里的位置**：它让「调行为」便宜到一张消费级显卡就能做，但表达能力受 rank 限制，不适合「学大量新知识」。这和上面的规则一致——知识走 RAG，行为才微调。
...
| 选项 | 适合什么 | 代价 |
|---|---|---|
| 动态 few-shot | 案例之间差异大，一组固定示例覆盖不住 | 检索一层相似案例当示例，等于给 few-shot 加了个检索器（第 15 课的机制） |
...
还有一处默认值容易被忽略：`API_USD_PER_M_IN` 填的是同步调用的单价。**能接受几小时延迟的活，走供应商的[批处理接口](https://platform.openai.com/docs/guides/batch)，单价通常低一半**（各家都标在 50% 上下，以文档为准），临界点跟着右移一倍——很多算下来「该自建了」的场景，改成批量提交之后就不必自建。哪些活能这么跑：离线评测、批量打标、文档预处理与向量重建（第 17 课）、蒸馏用的数据生成。判断标准只有一条：**这个结果有没有人在等**。
...
- **微调数据本身是资产也是风险。** 训练数据里的 PII 会被模型记住；数据的来源和授权要能说清（第 22 课 LLM04）。
