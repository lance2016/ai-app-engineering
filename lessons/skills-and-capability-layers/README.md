...
<summary>例子：装了 30 个 Skill，用户问一句「今天几号」，输入先花掉一万四千 token</summary>

一个内部 Agent，半年里陆续加了 30 个 Skill：报销审核、周报模板、值班表查询、发票抽取……每个 Skill 是一份两三百行的 Markdown，装载方式是启动时把全部正文拼进 system prompt。

用户问了一句「今天几号」，这次请求的输入是这样：
...
回答只用了 12 个输出 token。而且答得不太对劲——模型在日期后面顺手加了一句「如果这是要填进报销单，注意本月 25 日截止」。报销 Skill 的正文一直待在窗口里，模型分不清哪一份跟眼前这句话有关。
