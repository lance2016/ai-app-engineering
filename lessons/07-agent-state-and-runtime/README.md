...
主项目 [M2 数据与状态](../../project/m2-state-and-storage/README.md) 把 `thread.py` 的 `save()` / `load()` 换成 PostgreSQL 的 `conversation` / `message` 表，`status()` 和 `pending_tool_calls()` 变成查询。这一课的 `run()` 是 M3 `ToolRunner` 的骨架。
