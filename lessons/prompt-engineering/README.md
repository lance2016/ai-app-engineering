...
到这里都还顺。问题在下一次：线上此刻跑的是改之前那版还是改之后那版，查得出来吗？这次加的两个示例，会不会把别的用例带坏？
...
运营报来的那个 case 进了第三条，这是固定样例通常的长法：线上漏出来一个，就补一条进去。v1 在它上面拿 4/5，v2 拿 5/5，这才是「加两个示例值不值那一倍 token」的依据。
