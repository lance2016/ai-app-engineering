...
# 原则 06｜副作用要能安全重试、可确认、可审计

> 读操作出错通常可以重来，副作用出错可能就是事故。凡是会改变外部世界的动作，运行时必须先定义重试边界：能幂等就幂等，不能幂等就查状态、对账或交给人处理；高风险动作要先确认，事后要能审计。

## 主张

Agent 的动作分两类：只读的和有副作用的。只读动作失败了通常可以再试；副作用不一样，转一笔钱、发一封邮件、删一个文件，多做一次就是多一次损失。所以副作用要额外满足三件事：

1. **优先幂等**：同一个意图执行多次，效果和执行一次相同。运行时无法保证「执行」和「记录执行结果」原子完成，进程可能在两者之间崩掉。外部接口不支持幂等时，不能假装重试安全，要改用查询、对账队列或人工确认处理未知状态。
...
- 相关课程：[05 Tool Calling](../lessons/tool-calling/README.md)（幂等键与确认门的代码）、[07 Agent State 与 Runtime](../lessons/agent-state-and-runtime/README.md)（暂停恢复与事件记录）、[21 可靠性、成本、部署与 LLMOps](../lessons/reliability-cost-llmops/README.md)（重试策略）
