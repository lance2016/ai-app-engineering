...
> 在应用层，模型服务通常不替你持有任务状态，输出取决于这次请求实际收到的输入。如果你看不到、改不了发给模型的每一个字节，你就没有在做工程，只是在祈祷。
...
12-factor-agents 把这两条分别列为 factor 02 和 factor 03。它还指出一件容易被忽略的事：你甚至不必用标准的 system / user / assistant 消息格式。把整段历史打包成一条消息、换成自定义的结构化格式，往往更省 token，也更集中注意力。要不要这么做另说，但你得有这个自由。
